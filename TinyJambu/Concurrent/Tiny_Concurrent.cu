		...
		if (z != 3) {
			...
			for (int i = 64; i < 1025; i *= 2) {
				...
				for (int a = 1; a < 4; a++) {
					...
					kernel = ((a == 1) ? &crypto_aead_encrypt_gpu_global_OpRef : ((a == 2) ? &crypto_aead_encrypt_gpu_global_Op : &crypto_aead_encrypt_gpu_global_Op_KeyInversion));

					char* kernelName = ((a == 1) ? "GPU Ref    " : ((a == 2) ? "GPU Op Reg " :  "GPU Op Inv  "));
