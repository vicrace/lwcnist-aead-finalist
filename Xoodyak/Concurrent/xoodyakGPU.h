...
__device__ static void Xoodoo_RoundG__Fine(tXoodooLane* a, tXoodooLane rc)
{
	unsigned int x, y;
	__shared__ tXoodooLane    b[NLANES];
	__shared__ tXoodooLane    p[NCOLUMS];
	...
	int c = (innertid * fineLevel * ceil(((double)NCOLUMS / (double)fineLevel)));
	int f = (c + ceil(((double)NCOLUMS / (double)fineLevel)));
	f = (f > NCOLUMS) ? NCOLUMS : f; //prevent outofboudary
...
__device__ static  void Xoodoo_Permute_NroundsG__Fine(void* state, uint32_t nr)
{
	tXoodooLane        a[NLANES];
	unsigned int    i;
	...
	for (i = MAXROUNDS - nr; i < MAXROUNDS; ++i) {
		Xoodoo_RoundG__Fine(a, RCG[i]);
	}
	fromWordsToBytesG((unsigned char*)state, a);

}

__device__ static void Xoodyak_UpG__Fine(Xoodyak_Instance* instance, uint8_t* Yi, unsigned int YiLen, uint8_t Cu)
{
	if (instance->mode != Cyclist_ModeHash) {
		((unsigned char*)instance->state)[Xoodoo_stateSizeInBytes - 1] ^= Cu;
	}
	Xoodoo_Permute_NroundsG__Fine(&(instance->state), PERMUTATION_ROUND);
	instance->phase = Cyclist_PhaseUp;
...
__device__ static void Xoodyak_AbsorbAnyG__Fine(Xoodyak_Instance* instance, const uint8_t* X, size_t XLen, unsigned int r, uint8_t Cd)
{
	unsigned int splitLen;

	do {
		if (instance->phase != Cyclist_PhaseUp) {
			Xoodyak_UpG__Fine(instance, NULL, 0, 0);
		}
		splitLen = MyMin(XLen, r);
		Xoodyak_DownG(instance, X, splitLen, Cd);
		Cd = 0;
...
__device__ static void Xoodyak_AbsorbKeyG__Fine(Xoodyak_Instance* instance, const uint8_t* K, size_t KLen, const uint8_t* ID, size_t IDLen, const uint8_t* counter, size_t counterLen)
{
	uint8_t KID[Cyclist_Rkin];
	...
	if (KLen != 0) {
		...
		memcpy(KID + KLen, ID, IDLen);
		KID[KLen + IDLen] = (uint8_t)IDLen;
		Xoodyak_AbsorbAnyG__Fine(instance, KID, KLen + IDLen + 1, instance->Rabsorb, 0x02);
		if (counterLen != 0) {
			Xoodyak_AbsorbAnyG__Fine(instance, counter, counterLen, 1, 0x00);
		}
	}
}

__device__ void Xoodyak_InitializeG__Fine(Xoodyak_Instance* instance, const uint8_t* K, size_t KLen, const uint8_t* ID, size_t IDLen, const uint8_t* counter, size_t counterLen)
{
	Xoodoo_InitializeG(instance->state);
	instance->phase = Cyclist_PhaseUp;
	instance->mode = Cyclist_ModeHash;
	...
	if (KLen != 0) {
		Xoodyak_AbsorbKeyG__Fine(instance, K, KLen, ID, IDLen, counter, counterLen);
	}
}


__device__ static void Xoodyak_CryptG__Fine(Xoodyak_Instance* instance, const uint8_t* I, uint8_t* O, size_t IOLen, int decrypt)
{
	unsigned int splitLen;
	uint8_t      P[Cyclist_Rkout];
	uint8_t      Cu = 0x80;
	...
	do {
		splitLen = MyMin(IOLen, Cyclist_Rkout); /* use Rkout instead of Rsqueeze, this function is only called in keyed mode */
		if (decrypt != 0) {
			Xoodyak_UpG__Fine(instance, NULL, 0, Cu); /* Up without extract */
			Xoodoo_ExtractAndAddBytesG(instance->state, I, O, 0, splitLen); /* Extract from Up and Add */
			Xoodyak_DownG(instance, O, splitLen, 0x00);
		}
		else {
...
__device__ void Xoodyak_EncryptG__Fine(Xoodyak_Instance* instance, const uint8_t* P, uint8_t* C, size_t PLen)
{
	Xoodyak_CryptG__Fine(instance, P, C, PLen, 0);
}

__device__ void Xoodyak_AbsorbG__Fine(Xoodyak_Instance* instance, const uint8_t* X, size_t XLen)
{
	Xoodyak_AbsorbAnyG__Fine(instance, X, XLen, instance->Rabsorb, 0x03);
}


__device__ static void Xoodyak_SqueezeAnyG__Fine(Xoodyak_Instance* instance, uint8_t* Y, size_t YLen, uint8_t Cu)
{
	unsigned int len;

	len = MyMin(YLen, instance->Rsqueeze);
	Xoodyak_UpG__Fine(instance, Y, len, Cu);
	Y += len;
	YLen -= len;
	while (YLen != 0) {
		Xoodyak_DownG(instance, NULL, 0, 0);
		len = MyMin(YLen, instance->Rsqueeze);
		Xoodyak_UpG__Fine(instance, Y, len, 0);
		Y += len;
		YLen -= len;
	}
}
