...
//GPU optimised version - use array
__global__ void crypto_aead_encrypt_gpu_global_Op_Arr(uint8_t* c, uint64_t* clen, const uint8_t* m, uint64_t mlen, const uint8_t* ad, uint64_t adlen,
	const uint8_t* nsec, const uint8_t* npub, const uint8_t* k) {
	(void)nsec;
	...
	*clen = mlen + CRYPTO_ABYTES;
	...
	if (adlen) {
		/* full associated data blocks */
		while (adlen >= ASCON_128_RATE) {
			x[0] ^= LOADBYTESG(ad + idx_ia, 8);
			P6_GO_ARR;
			ad += ASCON_128_RATE;
			adlen -= ASCON_128_RATE;
		}
		/* final associated data block */
		x[0] ^= LOADBYTESG(ad + idx_ia, adlen);
		x[0] ^= PAD(adlen);
		P6_GO_ARR;
	}
	/* domain separation */
	...
	/* full plaintext blocks */
	while (mlen >= ASCON_128_RATE) {
		x[0] ^= LOADBYTESG(m + idx_im, 8);
		STOREBYTESG(c + idx_out, x[0], 8);
		P6_GO_ARR;
		m += ASCON_128_RATE;
		c += ASCON_128_RATE;
		mlen -= ASCON_128_RATE;
	...
	/* final plaintext block */
	x[0] ^= LOADBYTESG(m + idx_im, mlen);
	STOREBYTESG(c + idx_out, x[0], mlen);
	x[0] ^= PAD(mlen);
	c += mlen;
	...
	/* set tag */
	STOREBYTESG(c + idx_out, x[3], 8);
	STOREBYTESG(c + idx_out + 8, x[4], 8);
}
...
//GPU optimised version - use array
__global__ void crypto_aead_encrypt_gpu_global_Op_Arr_Trans(uint8_t* c, uint64_t* clen, const uint8_t* m, uint64_t mlen, const uint8_t* ad, uint64_t adlen,
	const uint8_t* nsec, const uint8_t* npub, const uint8_t* k) {
	(void)nsec;
	...
	*clen = mlen + CRYPTO_ABYTES;
	...
	if (adlen) {
		/* full associated data blocks */
		while (adlen >= ASCON_128_RATE) {
			x[0] ^= LOADBYTESG(ad + tao, 8);
			P6_GO_ARR;
			ad += ASCON_128_RATE;
			adlen -= ASCON_128_RATE;
		}
		/* final associated data block */
		x[0] ^= LOADBYTESG(ad + tao, adlen);
		x[0] ^= PAD(adlen);
		P6_GO_ARR;
	}
	/* domain separation */
	...
	/* full plaintext blocks */
	while (mlen >= ASCON_128_RATE) {
		x[0] ^= LOADBYTESG(m + tmo, 8);
		STOREBYTESG(c + tci, x[0], 8);
		P6_GO_ARR;
		m += ASCON_128_RATE;
		c += ASCON_128_RATE;
		mlen -= ASCON_128_RATE;
	...
	/* final plaintext block */
	x[0] ^= LOADBYTESG(m + tmo, mlen);
	STOREBYTESG(c + tci, x[0], mlen);
	x[0] ^= PAD(mlen);
	c += mlen;
	...
	/* set tag */
	STOREBYTESG(c + tci, x[3], 8);
	STOREBYTESG(c + tci + 8, x[4], 8);
}
	...
	*d_clen = MAX_CIPHER_LENGTH;
	...
	for (int i = 64; i < 1025; i *= 2) {
		...
		for (int z = 1; z < 6; z++) {
			...
			kernel = ((z == 1) ? &crypto_aead_encrypt_gpu : ((z == 2) ? &crypto_aead_encrypt_gpu_global_Op : ((z == 3) ? &crypto_aead_encrypt_gpu_global_Op_Arr : 
				((z == 4) ? &crypto_aead_encrypt_gpu_global_Op_Trans: &crypto_aead_encrypt_gpu_global_Op_Arr_Trans ))));
			char* kernelName = ((z == 1) ? "GPU Unoptimised" : ((z == 2) ? "GPU Op   " : ((z == 3) ? "GPU Op Arr" : ((z == 4) ? "GPU OpTrans " : "GPU OpArrT" ))));
