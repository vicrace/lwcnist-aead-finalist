		...
		if (z != 3) {
			...
			for (int i = 64; i < 1025; i *= 2) {
				...
				for (int a = 1; a <= 3; a++) {
					...
					kernel = ((a == 1) ? &crypto_aead_encrypt_gpu : ((a == 2) ? &crypto_aead_encrypt_gpu_global_Op_Trans : &crypto_aead_encrypt_gpu_global_Op_Arr_Trans));
					char* kernelName = ((a == 1) ? "GPU Unoptimised" : ((a == 2)? "GPU OpTrans " : "GPU OpArrT "));
