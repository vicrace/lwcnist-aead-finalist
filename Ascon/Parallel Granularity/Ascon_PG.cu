	...
	for (int z = 0; z < BATCH_SIZE; z++) {
		...
		*d_clen = MAX_CIPHER_LENGTH;
		...
		for (int i = 1; i < 1025; i *= 2) {
			...
			cudaEventRecord(start, 0);
			crypto_aead_encrypt_gpu_global_Op_Arr << <BATCH[z] / i, i >> > (d_c, d_clen, d_m, mlen, d_a, alen, 0, d_n, d_k);
			cudaEventRecord(stop, 0);
			cudaEventSynchronize(stop);
			kernel = 0;
			memcpy_d2h=0;
			...
			checkResult("GPU Op Arr", ct, h_c, MAX_CIPHER_LENGTH, BATCH[z]);
#ifdef WRITEFILE
			PrintTime(ct, h_c, d_clen, i, memcyp_h2d, kernel, memcpy_d2h, cpu_t, fpt, BATCH[z], "GPU Op");
#else
			PrintTime(ct, h_c, d_clen, i, memcyp_h2d, kernel, memcpy_d2h, cpu_t, NULL, BATCH[z], "GPU Op Arr");
#endif
			memset(h_c, 0, BATCH[z] * clen * sizeof(uint8_t));
