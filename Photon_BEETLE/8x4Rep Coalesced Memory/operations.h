...
static void PrintTime(uint8_t* ct, uint8_t* h_c, uint64_t* d_clen, int threads, float memcpyH2D, float kernelTime, float memcpyD2H, float cpu_t, FILE * fpt, uint8_t* tag, uint8_t* h_t, char* name ) {

	constexpr size_t tlen = 16ul;

	checkResult("Cipher", ct, h_c, MAX_CIPHER_LENGTH);
#ifdef PRINTC
